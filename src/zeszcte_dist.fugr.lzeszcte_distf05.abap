...
*----------------------------------------------------------------------*
TABLES: ZESZDE_CTE_DIST_VT_ALV,
        ZESZIB_CTE_DIST_NT_ALV.
...
DATA: IT_05_NIT_AJUSTE   TYPE TABLE OF ZESZIB_CTE_DIST_NIT WITH HEADER LINE.
...
DATA: IT_05_VT                TYPE TABLE OF ZESZDE_CTE_DIST_VT_ALV WITH HEADER LINE,
      IT_05_DUP               TYPE TABLE OF ZESZIB_CTE_DIST_DUP    WITH HEADER LINE,
      IT_05_N55               TYPE TABLE OF ZESZIB_CTE_DIST_N55    WITH HEADER LINE,
      IT_05_N01               TYPE TABLE OF ZESZIB_CTE_DIST_N01    WITH HEADER LINE,
      IT_05_NIT               TYPE TABLE OF ZESZIB_CTE_DIST_NIT    WITH HEADER LINE,
      IT_05_NF_ALV            TYPE TABLE OF ZESZIB_CTE_DIST_NT_ALV WITH HEADER LINE,
      QT_LINHAS_VT            TYPE I,
...
FIELD-SYMBOLS: <FS_05_N55> TYPE ZESZIB_CTE_DIST_N55,
               <FS_05_N01> TYPE ZESZIB_CTE_DIST_N01,
               <FS_05_NIT> TYPE ZESZIB_CTE_DIST_NIT,
               <FS_05_VT>  TYPE ZESZDE_CTE_DIST_VT_ALV.
...
FORM HANDLE_HOTSPOT_CLICK_0303
  ...
  READ TABLE IT_05_VT INTO ZESZDE_CTE_DIST_VT_ALV INDEX ROW_ID.
...
FORM YCTE_FATURAR_INFO  USING    P_I_ZIB_CTE TYPE REF TO ZCL_CTE_PAGAMENTO
  ...
  LC_ZIB_CTE->ZIF_CADASTRO~GET_REGISTRO( IMPORTING E_REGISTRO = ZESZIB_CTE_DIST_TER ).
  ...
  "Distribuir Valores
  IF ZESZIB_CTE_DIST_TER-CK_FINALIZADO EQ ABAP_FALSE AND LC_ZIB_CTE->GET_CK_AUTORIZADO_PAGAMENTO( ) EQ ABAP_TRUE.
  ...
  ELSEIF QT_LINHAS_VT EQ 1.
    LC_INDEX_ATUAL = 1.
    LC_INDEX = P_I_INDEX.
    READ TABLE IT_05_VT INTO ZESZDE_CTE_DIST_VT_ALV INDEX LC_INDEX_ATUAL.
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    CALL SCREEN 0304 STARTING AT 15 05.
...
MODULE STATUS_0304 OUTPUT.
  ...
  IF LC_ALTEROU EQ ABAP_TRUE.
    ...
    LC_ZIB_CTE->SET_ZBVTYP( EXPORTING I_ZBVTYP = ZESZIB_CTE_DIST_TER-ZBVTYP ).
    LC_ZIB_CTE->SET_ZDT_VENCTO( EXPORTING I_ZDT_VENCTO = ZESZIB_CTE_DIST_TER-ZESZDT_VENCTO ).
    LC_ZIB_CTE->SET_MWSKZ( EXPORTING I_MWSKZ = ZESZIB_CTE_DIST_TER-MWSKZ ).
    LC_ZIB_CTE->SET_DT_CHEGADA( EXPORTING I_DT_CHEGADA = ZESZIB_CTE_DIST_TER-DT_CHEGADA ).
  ENDIF.
...
*  3  CT-e Substituto
        ...
        LOOP AT SCREEN.
          IF ( SCREEN-INPUT NE 0 ) AND
             (
               ...
               SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-ZESZVLR_FRETE' OR
               SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_ORIGEM' OR
               SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
            SCREEN-INPUT = 0.
            MODIFY SCREEN.
          ENDIF.
        ENDLOOP.
    ...
    IF ZESZIB_CTE_DIST_TER-CD_MODAL EQ '04'.
      LOOP AT SCREEN.
        IF ( SCREEN-INPUT NE 0 ) AND
           ( SCREEN-NAME EQ 'ZESZIB_CTE_DIST_TER-DT_CHEGADA' OR SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
          SCREEN-INPUT = 0.
          MODIFY SCREEN.
        ENDIF.
      ENDLOOP.
      ...
      WHEN '03'.
        ...
        LOOP AT SCREEN.
          IF SCREEN-INPUT NE '0'.
            SCREEN-INPUT = '0'.
          ENDIF.
          IF SCREEN-NAME EQ 'ZESZIB_CTE_DIST_TER-DT_CHEGADA'.
            SCREEN-INPUT = '1'.
          ENDIF.
          MODIFY SCREEN.
        ENDLOOP.
      ...
      WHEN OTHERS.
        ...
        IF ( ( LC_ZIB_CTE->GET_QT_REG_NF( EXPORTING I_TKNUM  = LC_TKNUM  ) EQ 1 AND
          ...
          IF LC_ALTEROU_PESO_CHEGADA EQ ABAP_TRUE.
            ...
            IT_05_NIT-ZVLR_VI         = ZESZDE_CTE_DIST_VT_ALV-ZVLR_VI.
            IT_05_NIT-ZESZVLR_FRETE      = ZESZDE_CTE_DIST_VT_ALV-ZESZVLR_FRETE.
            IT_05_NIT-PESO_ORIGEM     = ZESZDE_CTE_DIST_VT_ALV-PESO_ORIGEM.
            IT_05_NIT-PESO_CHEGADA    = ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA.
            IT_05_NIT-ZVLR_MERCADORIA = ZESZDE_CTE_DIST_VT_ALV-ZVLR_MERCADORIA.
            ...
            IT_05_DUP[] = LC_ZIB_CTE->GET_IT_DUP( ).
            IT_05_VT[]  = LC_ZIB_CTE->GET_IT_VT( ).
            READ TABLE IT_05_VT INTO ZESZDE_CTE_DIST_VT_ALV WITH KEY TKNUM = LC_TKNUM.
          ELSE.
            IT_05_VT[]  = LC_ZIB_CTE->GET_IT_VT( ).
            READ TABLE IT_05_VT INTO ZESZDE_CTE_DIST_VT_ALV WITH KEY TKNUM = LC_TKNUM.
          ENDIF.
          ...
          IF LC_ZIB_CTE->GET_CK_FATURA_PELA_VT( ) EQ ABAP_TRUE.
            LOOP AT SCREEN.
              IF ( SCREEN-INPUT NE 1 ) AND
                 ( SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-ZESZVLR_FRETE' OR
                   SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-ZVLR_VI' OR
                   SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_ORIGEM' ).
                SCREEN-INPUT = 1.
                MODIFY SCREEN.
              ENDIF.
            ENDLOOP.
          ...
          LOOP AT SCREEN.
            IF ( SCREEN-INPUT NE 1 ) AND
               ( SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-DT_CHEGADA' OR
                 SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
              SCREEN-INPUT = 1.
              MODIFY SCREEN.
            ENDIF.
        ...
        ELSEIF LC_ZIB_CTE->GET_CK_ENTRADA_MANUAL_PESO( ) EQ ABAP_FALSE.
          ...
          LOOP AT SCREEN.
            IF ( SCREEN-INPUT NE 0 ) AND ( SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA' OR SCREEN-NAME EQ 'ZESZDE_CTE_DIST_VT_ALV-DT_CHEGADA' ).
              SCREEN-INPUT = 0.
              MODIFY SCREEN.
            ENDIF.
            IF ( SCREEN-INPUT NE 0 ) AND ( SCREEN-NAME EQ 'BTN_NOTAS' ).
...
MODULE USER_COMMAND_0304 INPUT.
  ...
  CASE OK_CODE.
    ...
    WHEN OK_BTN_NOTAS.
      ...
      PERFORM INFORMA_DADOS_POR_NOTA USING ZESZDE_CTE_DIST_VT_ALV.

    WHEN OK_CONFIRMAR OR OK_SALVAR.

      IF ZESZDE_CTE_DIST_VT_ALV-ZESZQUEBRA GT 0 AND ZESZDE_CTE_DIST_VT_ALV-ZESZVLR_QUEBRA EQ 0.
      ...
      IF ZESZDE_CTE_DIST_VT_ALV-PESO_CHEGADA IS INITIAL AND LC_ZIB_CTE->GET_CK_ENTRADA_MANUAL_PESO( ) EQ ABAP_FALSE.
      ...
      ZESZDE_CTE_DIST_VT_ALV-CK_PESO_DIGITADO = ABAP_TRUE.
      CLEAR: LC_ORDEM.
...
*----------------------------------------------------------------------*
FORM INFORMA_DADOS_POR_NOTA  USING P_VT_ALV TYPE ZESZDE_CTE_DIST_VT_ALV.
  ...
  IF LC_ZIB_CTE->GET_QT_REG_NF( EXPORTING I_TKNUM = P_VT_ALV-TKNUM ) = 1.
    READ TABLE IT_05_NF_ALV INTO ZESZIB_CTE_DIST_NT_ALV INDEX 1.
    PERFORM SELECIONA_ITENS_DA_NOTA USING ZESZIB_CTE_DIST_NT_ALV-DOCNUM.
  ...
  READ TABLE IT_05_VT INTO ZESZDE_CTE_DIST_VT_ALV WITH KEY TKNUM = P_VT_ALV-TKNUM.
...
MODULE STATUS_0306 OUTPUT.
  ...
  SET TITLEBAR 'TL0306' WITH ZESZIB_CTE_DIST_NT_ALV-TKNUM ZESZIB_CTE_DIST_NT_ALV-DOCNUM.
...
FORM FILL_IT_FIELDCATALOG_0306 .
  ...
  CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
    EXPORTING
      I_STRUCTURE_NAME = 'ZESZIB_CTE_DIST_NIT'
    CHANGING
  ...
  LOOP AT IT_CATALOG_0306 ASSIGNING <FS_CAT_0306>.
    ...
    CASE <FS_CAT_0306>-FIELDNAME.
      WHEN 'ZVLR_VI' OR 'ZESZVLR_FRETE' OR 'PESO_ORIGEM' OR 'PESO_CHEGADA'.
...
FORM POPULA_NOTAS_TKNUM  USING  P_TKNUM TYPE TKNUM.
  ...
  DATA: WA_DIST_N55 TYPE ZESZIB_CTE_DIST_N55,
        WA_DIST_N01 TYPE ZESZIB_CTE_DIST_N01.
  ...
  LOOP AT IT_05_N55 INTO WA_DIST_N55 WHERE TKNUM = P_TKNUM.
    CLEAR: ZESZIB_CTE_DIST_NT_ALV.
    MOVE-CORRESPONDING WA_DIST_N55 TO ZESZIB_CTE_DIST_NT_ALV.
    ZESZIB_CTE_DIST_NT_ALV-DOCNUM    = WA_DIST_N55-DOCNUM_NFE.
    ZESZIB_CTE_DIST_NT_ALV-IC_EDITAR = ICON_CHANGE_TEXT.
    APPEND ZESZIB_CTE_DIST_NT_ALV TO IT_05_NF_ALV.
  ENDLOOP.

  LOOP AT IT_05_N01 INTO WA_DIST_N01 WHERE TKNUM = P_TKNUM.
    CLEAR: ZESZIB_CTE_DIST_NT_ALV.
    MOVE-CORRESPONDING WA_DIST_N01 TO ZESZIB_CTE_DIST_NT_ALV.
    ZESZIB_CTE_DIST_NT_ALV-DOCNUM    = WA_DIST_N01-DOCNUM_NF.
    ZESZIB_CTE_DIST_NT_ALV-IC_EDITAR = ICON_CHANGE_TEXT.
    APPEND ZESZIB_CTE_DIST_NT_ALV TO IT_05_NF_ALV.
  ENDLOOP.
...
FORM HANDLE_HOTSPOT_CLICK_0305
  ...
  READ TABLE IT_05_NF_ALV INTO ZESZIB_CTE_DIST_NT_ALV INDEX ROW_ID.
...
FORM FILL_IT_FIELDCATALOG_0305 .
  ...
  CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
    EXPORTING
      I_STRUCTURE_NAME = 'ZESZIB_CTE_DIST_NT_ALV'
    CHANGING

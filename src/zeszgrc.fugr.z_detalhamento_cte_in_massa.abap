...
*"*"Interface local:
*"  IMPORTING
*"     VALUE(I_CHAVE_NFE) TYPE  ZESZCTMM_CHAVE_NFE
*"     VALUE(I_BUKRS) TYPE  BUKRS
*"     VALUE(I_BRANCH) TYPE  J_1BBRANC_
*"     VALUE(I_MODEL) TYPE  J_1BMODEL
*"  EXPORTING
*"     VALUE(E_XML_CTE) TYPE  ZESZCTMM_XML_CTE
*"----------------------------------------------------------------------
  ...
  DATA: t_xstring       TYPE TABLE OF zde_s_xstring,
        ...
        wl_xml_cte      TYPE ZESZCTE_XML_SEFAZ_AUTH,
        wl_zde_xml_cte  TYPE ZESZDE_XML_CTE,
        t_zctmm_xml_cte TYPE ZESZCTMM_XML_CTE.

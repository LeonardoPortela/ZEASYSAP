CLASS ZESZCL_WEBSERVICE DEFINITION
  PUBLIC
  CREATE PUBLIC .

*"* public components of class ZESZCL_WEBSERVICE
*"* do not include other source files here!!!
  PUBLIC SECTION.

    INTERFACES ZESZIF_WEBSERVICE .
    ...
    METHODS set_servico
      IMPORTING
        !i_servico TYPE ZESZTIPOWEBSERV
      RAISING
        ZESZCX_WEBSERVICE .
    METHODS get_servico
      RETURNING
        VALUE(e_servico) TYPE ZESZTIPOWEBSERV .
    METHODS set_tipo
      IMPORTING
        !i_tipo TYPE ZESZTIPOWEBADM .
    METHODS get_tipo
      RETURNING
        VALUE(e_tipo) TYPE ZESZTIPOWEBADM .
    METHODS get_senha
      RETURNING
        VALUE(e_senha) TYPE ZESZDE_UI_SRC_PASSWORD60
      RAISING
        ZESZCX_WEBSERVICE .
    METHODS get_uri
      RETURNING
        VALUE(e_uri) TYPE ZESZURLWEBADM
      RAISING
        ZESZCX_WEBSERVICE .
    METHODS get_usuario
      RETURNING
        VALUE(e_usuario) TYPE ZESZDE_USUARIO
      RAISING
        ZESZCX_WEBSERVICE .
    METHODS url
      IMPORTING
        !i_url        TYPE ZESZURLWEBADM OPTIONAL
  ...
  PROTECTED SECTION.
*"* protected components of class ZESZCL_WEBSERVICE
*"* do not include other source files here!!!
private section.

  aliases AUTENTICA_MODULE
    for ZESZIF_WEBSERVICE~AUTENTICA_MODULE .

*"* private components of class ZESZCL_WEBSERVICE
*"* do not include other source files here!!!
  data AT_SERVICO type ZESZTIPOWEBSERV .
  data AT_TIPO type ZESZTIPOWEBADM .
  data AT_HTTP type ref to IF_HTTP_CLIENT .
  data AT_CONTENT_TYPE type ZESZDE_WEB_SERV_CTX .
...
CLASS ZESZCL_WEBSERVICE IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    CLEAR: me->at_servico,
           ...
           me->at_http,
           me->at_content_type,
           me->ZESZIF_WEBSERVICE~autentica_opus,
           me->ZESZIF_WEBSERVICE~nm_auth_webservice,
           me->ZESZIF_WEBSERVICE~ck_usa_auth_webservice.

    me->ZESZIF_WEBSERVICE~autentica_opus = abap_false.
    me->ZESZIF_WEBSERVICE~ck_usa_auth_webservice = abap_false.
  ...
  METHOD get_senha.
    ...
    ELSE.
      ...
      CASE me->ZESZIF_WEBSERVICE~ck_usa_auth_webservice.
        WHEN abap_false.
          ...
          IF NOT ( var_servico IS INITIAL ) AND NOT ( var_tipo IS INITIAL ).
            ...
            SELECT SINGLE * FROM ZESZCIOT_WEBSERVICE
            ...
            ELSE.
              RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>url_nao_encontrado.
            ENDIF.
          ENDIF.
        ...
        WHEN abap_true.
          ...
          SELECT SINGLE * INTO @DATA(wa_zauth_webservice)
            FROM ZESZAUTH_WEBSERVICE
           WHERE service EQ @me->ZESZIF_WEBSERVICE~nm_auth_webservice.
...
************************************
    ...
    DATA:  gw_webservice TYPE ZESZCIOT_WEBSERVICE. "Work Area para guardar informações selecionadas da tabela ZESZCIOT_WEBSERVICE.
    ...
    SELECT SINGLE * FROM ZESZCIOT_WEBSERVICE INTO gw_webservice WHERE servico EQ i_servico.
    ...
    ELSE. "Se o serviço não for encontrado no cadastro enviar uma Exception para ser tratada.
      RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>servico_nao_encontrado.
    ENDIF.
...
************************************
    ...
    DATA: gw_webservice TYPE ZESZCIOT_WEBSERVICE. "Tabela de WebServices para Viagem Adiministradora
    DATA: var_servico TYPE ZESZTIPOWEBSERV, "Tipo de Serviço WebService
          var_tipo    TYPE ZESZTIPOWEBADM, "Tipo de WebService
      ...
      CASE me->ZESZIF_WEBSERVICE~ck_usa_auth_webservice.
        WHEN abap_false.
          ...
          IF NOT ( var_servico IS INITIAL ) AND NOT ( var_tipo IS INITIAL ).
            ...
            "Selecionar na tabela o serviço e tipo de serviço que vai ser utilizado.
            SELECT SINGLE * FROM ZESZCIOT_WEBSERVICE INTO gw_webservice WHERE servico EQ var_servico
                                                                       AND tipo    EQ var_tipo.
            ...
            IF ( sy-subrc EQ 0 ). "Caso o endereço seja encontrado.
              ...
              CASE sy-subrc.
                WHEN: 1.
                  ...
                  RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>argumento_nao_encontrado.
                WHEN: 2.
                  "Este erro lança uma expcetion de "Plugin não ativo", quando a comunicação HTTP não esteja ativa no SAP.
                  RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>plugin_nao_ativo.
                WHEN: 3.
                  "Este erro lança uma exception generica "Erro Interno" do SAP.
                  RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>erro_interno.
              ENDCASE.

            ELSE.
              "Quando uma URL não for encontrada no cadastro da ZLES0096 (tabela: ZESZCIOT_WEBSERVICE)
  ...
  METHOD ZESZIF_WEBSERVICE~abrir_conexao.
*****************
...
*****************
  ...
  METHOD ZESZIF_WEBSERVICE~add_token_api_ad_http_cliente.

    ZESZCL_WEBSERVICE=>ZESZIF_WEBSERVICE~get_token_api_ad(
     EXPORTING
  ...
  METHOD ZESZIF_WEBSERVICE~add_token_opus_http_cliente.

    ZESZCL_WEBSERVICE=>ZESZIF_WEBSERVICE~get_token_opus(
      EXPORTING
        i_url_destino              = i_url_destino
        i_url_token                = i_url_token
      IMPORTING
  ...
  METHOD ZESZIF_WEBSERVICE~consultar.
    ...
    IF me->ZESZIF_WEBSERVICE~autentica_opus EQ abap_true.

      me->ZESZIF_WEBSERVICE~get_token_opus(
        EXPORTING
    ...
    ELSEIF me->ZESZIF_WEBSERVICE~autentica_api_ad EQ abap_true.

      me->ZESZIF_WEBSERVICE~get_token_api_ad(
        EXPORTING
          i_url_destino              = me->at_url
          i_url_token                = me->at_url_token
  ...
  METHOD ZESZIF_WEBSERVICE~get_token_api_ad.

    DATA: ex_webservice_token TYPE REF TO ZESZCL_WEBSERVICE,
    ...
    ex_webservice_token->ZESZIF_WEBSERVICE~nm_auth_webservice = 'URL_TOKEN_API_AD'.

    SELECT SINGLE * INTO @DATA(wa_zauth_webservice)
        FROM ZESZAUTH_WEBSERVICE
       WHERE service EQ @ex_webservice_token->ZESZIF_WEBSERVICE~nm_auth_webservice.
...
*          module   = |SAP_{ sy-sysid }|
      ...
      ex_webservice_token->ZESZIF_WEBSERVICE~abrir_conexao( i_http = var_http ).
      ...
      ex_webservice_token->ZESZIF_WEBSERVICE~consultar(
        EXPORTING
          i_http                     = var_http
  ...
  METHOD ZESZIF_WEBSERVICE~get_token_opus.
    ...
    DATA: lc_mensagem TYPE ty_retorno_msg,
          ...
          lv_token    TYPE ZESZINTEGRACAO0001.  ""*-#160251-03.12.2024-#160251-JT-inicio
...
*-#160251-03.12.2024-#160251-JT-fim
    ...
    ex_webservice_token->ZESZIF_WEBSERVICE~ck_usa_auth_webservice = abap_true.
    ex_webservice_token->ZESZIF_WEBSERVICE~nm_auth_webservice = 'URL_TOKEN_OPUS_API'.
...
*-#160251-03.12.2024-#160251-JT-fim
  ...
  METHOD ZESZIF_WEBSERVICE~tela_exception.
*****************

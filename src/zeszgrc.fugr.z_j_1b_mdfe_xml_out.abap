...
*"     REFERENCE(ZEV_ERROR_STATUS) TYPE  ZESZXNFE_ERRSTATUS
...
*----------------------------------------------------------------------*
  ...
  DATA: ls_dhcont    TYPE string,
        ...
        e_time       TYPE  erzet,
        v_line_id    TYPE ZESZXNFE_LINEID,
...
**<<<------"185281 - NMS - FIM------>>>
        ...
        zit_mdfe_infcte               TYPE ZESZXNFE_IF_MDFE_INFCTE_300_T,
        wit_mdfe_infcte               TYPE ZESZXNFE_IF_MDFE_INFCTE_300_S,
        ...
        zit_mdfe_infunidtransp        TYPE ZESZXNFE_IF_CTE_INFUNIDTRANSP_T,
        zit_mdfe_infunidcarga         TYPE ZESZXNFE_IF_CTE_INFUNIDCARGA_T,
        ...
        ziv_text_id_lacre             TYPE ZESZXNFE_TEXT_REF,
        zis_mdfe_prodpred             TYPE zrsi_mdfe_prodpred,
        ziv_resend                    TYPE ZESZXNFE_RESEND.

  DATA: v_seqno_ref_cte_nfe TYPE ZESZXNFE_SEQ_NO.
...
*CONTINGENCIA MDF-E - JT - 06.05.2024 =================================
  ...
  CASE wk_header-transp_mode.
    WHEN '1'.
      ...
      LOOP AT it_zsdt0105 INTO DATA(wa_zsdt0105).
        ...
        SELECT SINGLE * FROM ZESZCTE_SEGURO INTO @DATA(wa_zcte_seguro)
        ...
        SELECT SINGLE * INTO @DATA(wa_ciot) FROM ZESZCTE_CIOT WHERE docnum EQ @wa_zsdt0105-docnum.
        IF sy-subrc IS NOT INITIAL.
          CONTINUE.
        ENDIF.

        SELECT SINGLE * INTO @DATA(wa_identifica) FROM ZESZCTE_IDENTIFICA WHERE docnum EQ @wa_zsdt0105-docnum.
        IF sy-subrc IS NOT INITIAL.
          CONTINUE.
        ENDIF.
        ...
        SELECT SINGLE * INTO @DATA(wa_parceiro)
          FROM ZESZCTE_PARCEIROS WHERE docnum EQ @wa_zsdt0105-docnum.
      ...
      DATA: rgplacas TYPE RANGE OF ZESZPC_VEICULO.
    ...
    WHEN '3'.
      ...
      DATA: wa_zlest0066  TYPE zlest0066,
            wa_zlest0061  TYPE ZESZLEST0061,
            ...
            it_zlest0056  TYPE TABLE OF ZESZLEST0056,
            wa_zlest0056  TYPE ZESZLEST0056,
  ...
  LOOP AT it_doc_eletronico INTO DATA(wa_doc_eletronico).
    ...
    IF wa_doc_eletronico-docnum IS NOT INITIAL AND wa_doc_eletronico-form IS NOT INITIAL.
      TRY .
          ZESZCL_DOC_ELETRONICO=>ZESZIF_DOC_ELETRONICO~get_instance( i_docnum =  wa_doc_eletronico-docnum
            )->set_registro( EXPORTING i_docnum = wa_doc_eletronico-docnum i_sem_bloqueio = abap_true
            )->get_ck_doc_nao_cancel(
            ).
        CATCH ZESZCX_DOC_ELETRONICO INTO DATA(ex_erro).    " .
          zev_error_status      = 'V'.
...
*** Produto predominante - NCM.
        SELECT SINGLE c~steuc, b~docnum
          FROM ZESZSDT0105 AS a
           INNER JOIN ZESZCTE_INFO_NOTA AS b
...
*** Componentes do Pagamento do Frete
        SELECT SINGLE * FROM ZESZCTE_CIOT INTO @DATA(el_ciot) WHERE docnum EQ @vl_docnum_ori.
        IF sy-subrc IS INITIAL.
**<<<------"192137 - NMS - INI------>>>
...
**<<<------"192137 - NMS - FIM------>>>
        SELECT SINGLE * FROM ZESZCTE_IDENTIFICA INTO @DATA(el_identifica) WHERE docnum EQ @vl_docnum_ori.
        IF sy-subrc IS INITIAL                 AND

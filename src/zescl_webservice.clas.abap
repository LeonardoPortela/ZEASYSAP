CLASS zescl_webservice DEFINITION
  PUBLIC
  CREATE PUBLIC .

*"* public components of class ZESCL_WEBSERVICE
*"* do not include other source files here!!!
  PUBLIC SECTION.

    INTERFACES zesif_webservice .
  ...
  PROTECTED SECTION.
*"* protected components of class ZESCL_WEBSERVICE
*"* do not include other source files here!!!
private section.

  aliases AUTENTICA_MODULE
    for ZESIF_WEBSERVICE~AUTENTICA_MODULE .

*"* private components of class ZESCL_WEBSERVICE
*"* do not include other source files here!!!
...
CLASS ZESCL_WEBSERVICE IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    CLEAR: me->at_servico,
           ...
           me->at_http,
           me->at_content_type,
           me->zesif_webservice~autentica_opus,
           me->zesif_webservice~nm_auth_webservice,
           me->zesif_webservice~ck_usa_auth_webservice.

    me->zesif_webservice~autentica_opus = abap_false.
    me->zesif_webservice~ck_usa_auth_webservice = abap_false.
  ...
  METHOD get_senha.
    ...
    ELSE.
      ...
      CASE me->zesif_webservice~ck_usa_auth_webservice.
        WHEN abap_false.
          ...
          IF NOT ( var_servico IS INITIAL ) AND NOT ( var_tipo IS INITIAL ).
            ...
            ELSE.
              RAISE EXCEPTION TYPE zescx_webservice EXPORTING textid = zescx_webservice=>url_nao_encontrado.
            ENDIF.
          ENDIF.
        ...
        WHEN abap_true.
          ...
          SELECT SINGLE * INTO @DATA(wa_zauth_webservice)
           ...
           WHERE service EQ @me->zesif_webservice~nm_auth_webservice.
...
************************************
    ...
    ELSE. "Se o serviço não for encontrado no cadastro enviar uma Exception para ser tratada.
      RAISE EXCEPTION TYPE zescx_webservice EXPORTING textid = zescx_webservice=>servico_nao_encontrado.
    ENDIF.
...
************************************
            ...
            IF ( sy-subrc EQ 0 ). "Caso o endereço seja encontrado.
              ...
              CASE sy-subrc.
                WHEN: 1.
                  ...
                  RAISE EXCEPTION TYPE zescx_webservice EXPORTING textid = zescx_webservice=>argumento_nao_encontrado.
                WHEN: 2.
                  "Este erro lança uma expcetion de "Plugin não ativo", quando a comunicação HTTP não esteja ativa no SAP.
                  RAISE EXCEPTION TYPE zescx_webservice EXPORTING textid = zescx_webservice=>plugin_nao_ativo.
                WHEN: 3.
                  "Este erro lança uma exception generica "Erro Interno" do SAP.
                  RAISE EXCEPTION TYPE zescx_webservice EXPORTING textid = zescx_webservice=>erro_interno.
              ENDCASE.
  ...
  METHOD zesif_webservice~abrir_conexao.
*****************
...
*****************
  ...
  METHOD zesif_webservice~add_token_api_ad_http_cliente.

    zescl_webservice=>zesif_webservice~get_token_api_ad(
     EXPORTING
  ...
  METHOD zesif_webservice~add_token_opus_http_cliente.

    zescl_webservice=>zesif_webservice~get_token_opus(
      EXPORTING
        i_url_destino              = i_url_destino
        i_url_token                = i_url_token
      IMPORTING
  ...
  METHOD zesif_webservice~consultar.
    ...
    IF me->zesif_webservice~autentica_opus EQ abap_true.

      me->zesif_webservice~get_token_opus(
        EXPORTING
    ...
    ELSEIF me->zesif_webservice~autentica_api_ad EQ abap_true.

      me->zesif_webservice~get_token_api_ad(
        EXPORTING
          i_url_destino              = me->at_url
          i_url_token                = me->at_url_token
  ...
  METHOD zesif_webservice~get_token_api_ad.

    DATA: ex_webservice_token TYPE REF TO zescl_webservice,
    ...
    ex_webservice_token->zesif_webservice~nm_auth_webservice = 'URL_TOKEN_API_AD'.

    SELECT SINGLE * INTO @DATA(wa_zauth_webservice)
        FROM zauth_webservice
       WHERE service EQ @ex_webservice_token->zesif_webservice~nm_auth_webservice.
...
*          module   = |SAP_{ sy-sysid }|
      ...
      ex_webservice_token->zesif_webservice~abrir_conexao( i_http = var_http ).
      ...
      ex_webservice_token->zesif_webservice~consultar(
        EXPORTING
          i_http                     = var_http
  ...
  METHOD zesif_webservice~get_token_opus.
...
*-#160251-03.12.2024-#160251-JT-inicio
    IF i_servico = 'RV' OR _gestao_token_api_amaggi EQ abap_true.
      DATA(lt_header_auth) = zescl_gestao_token=>get_token_valido( i_id_token = '0008' ).
...
*-#160251-03.12.2024-#160251-JT-fim
    ...
    ex_webservice_token->zesif_webservice~ck_usa_auth_webservice = abap_true.
    ex_webservice_token->zesif_webservice~nm_auth_webservice = 'URL_TOKEN_OPUS_API'.
...
*-#160251-03.12.2024-#160251-JT-inicio
      IF i_servico = 'RV' OR _gestao_token_api_amaggi EQ abap_true.
        ...
        zescl_gestao_token=>update_token( i_token = lv_token ).
      ENDIF.
*-#160251-03.12.2024-#160251-JT-fim
  ...
  METHOD zesif_webservice~tela_exception.
*****************

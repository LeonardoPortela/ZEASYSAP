...
DATA: IT_05_VT                TYPE TABLE OF ZESDE_CTE_DIST_VT_ALV WITH HEADER LINE,
...
FIELD-SYMBOLS: <FS_05_N55> TYPE ZIB_CTE_DIST_N55,
               ...
               <FS_05_VT>  TYPE ZESDE_CTE_DIST_VT_ALV.
...
FORM HANDLE_HOTSPOT_CLICK_0303
  ...
  READ TABLE IT_05_VT INTO ZESDE_CTE_DIST_VT_ALV INDEX ROW_ID.
...
FORM YCTE_FATURAR_INFO  USING    P_I_ZIB_CTE TYPE REF TO ZCL_CTE_PAGAMENTO
  ...
  ELSEIF QT_LINHAS_VT EQ 1.
    LC_INDEX_ATUAL = 1.
    LC_INDEX = P_I_INDEX.
    READ TABLE IT_05_VT INTO ZESDE_CTE_DIST_VT_ALV INDEX LC_INDEX_ATUAL.
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    "Chamar Tela Individual """"""""""""""""""""""""""""""""""""""""""""""""
    CALL SCREEN 0304 STARTING AT 15 05.
...
*  3  CT-e Substituto
        ...
        LOOP AT SCREEN.
          IF ( SCREEN-INPUT NE 0 ) AND
             (
               ...
               SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-ZVLR_FRETE' OR
               SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_ORIGEM' OR
               SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
            SCREEN-INPUT = 0.
            MODIFY SCREEN.
          ENDIF.
        ENDLOOP.
    ...
    IF ZIB_CTE_DIST_TER-CD_MODAL EQ '04'.
      LOOP AT SCREEN.
        IF ( SCREEN-INPUT NE 0 ) AND
           ( SCREEN-NAME EQ 'ZIB_CTE_DIST_TER-DT_CHEGADA' OR SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
          SCREEN-INPUT = 0.
          MODIFY SCREEN.
        ENDIF.
      ENDLOOP.
      ...
      WHEN OTHERS.
        ...
        IF ( ( LC_ZIB_CTE->GET_QT_REG_NF( EXPORTING I_TKNUM  = LC_TKNUM  ) EQ 1 AND
          ...
          IF LC_ALTEROU_PESO_CHEGADA EQ ABAP_TRUE.
            ...
            IT_05_NIT-ZVLR_VI         = ZESDE_CTE_DIST_VT_ALV-ZVLR_VI.
            IT_05_NIT-ZVLR_FRETE      = ZESDE_CTE_DIST_VT_ALV-ZVLR_FRETE.
            IT_05_NIT-PESO_ORIGEM     = ZESDE_CTE_DIST_VT_ALV-PESO_ORIGEM.
            IT_05_NIT-PESO_CHEGADA    = ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA.
            IT_05_NIT-ZVLR_MERCADORIA = ZESDE_CTE_DIST_VT_ALV-ZVLR_MERCADORIA.
            ...
            IT_05_DUP[] = LC_ZIB_CTE->GET_IT_DUP( ).
            IT_05_VT[]  = LC_ZIB_CTE->GET_IT_VT( ).
            READ TABLE IT_05_VT INTO ZESDE_CTE_DIST_VT_ALV WITH KEY TKNUM = LC_TKNUM.
          ELSE.
            IT_05_VT[]  = LC_ZIB_CTE->GET_IT_VT( ).
            READ TABLE IT_05_VT INTO ZESDE_CTE_DIST_VT_ALV WITH KEY TKNUM = LC_TKNUM.
          ENDIF.
          ...
          IF LC_ZIB_CTE->GET_CK_FATURA_PELA_VT( ) EQ ABAP_TRUE.
            LOOP AT SCREEN.
              IF ( SCREEN-INPUT NE 1 ) AND
                 ( SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-ZVLR_FRETE' OR
                   SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-ZVLR_VI' OR
                   SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_ORIGEM' ).
                SCREEN-INPUT = 1.
                MODIFY SCREEN.
              ENDIF.
            ENDLOOP.
          ...
          LOOP AT SCREEN.
            IF ( SCREEN-INPUT NE 1 ) AND
               ( SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-DT_CHEGADA' OR
                 SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA' ).
              SCREEN-INPUT = 1.
              MODIFY SCREEN.
            ENDIF.
        ...
        ELSEIF LC_ZIB_CTE->GET_CK_ENTRADA_MANUAL_PESO( ) EQ ABAP_FALSE.
          ...
          LOOP AT SCREEN.
            IF ( SCREEN-INPUT NE 0 ) AND ( SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA' OR SCREEN-NAME EQ 'ZESDE_CTE_DIST_VT_ALV-DT_CHEGADA' ).
              SCREEN-INPUT = 0.
              MODIFY SCREEN.
            ENDIF.
            IF ( SCREEN-INPUT NE 0 ) AND ( SCREEN-NAME EQ 'BTN_NOTAS' ).
...
MODULE USER_COMMAND_0304 INPUT.
  ...
  CASE OK_CODE.
    ...
    WHEN OK_BTN_NOTAS.
      ...
      PERFORM INFORMA_DADOS_POR_NOTA USING ZESDE_CTE_DIST_VT_ALV.

    WHEN OK_CONFIRMAR OR OK_SALVAR.

      IF ZESDE_CTE_DIST_VT_ALV-ZQUEBRA GT 0 AND ZESDE_CTE_DIST_VT_ALV-ZVLR_QUEBRA EQ 0.
      ...
      IF ZESDE_CTE_DIST_VT_ALV-PESO_CHEGADA IS INITIAL AND LC_ZIB_CTE->GET_CK_ENTRADA_MANUAL_PESO( ) EQ ABAP_FALSE.
      ...
      ZESDE_CTE_DIST_VT_ALV-CK_PESO_DIGITADO = ABAP_TRUE.
      CLEAR: LC_ORDEM.
...
*----------------------------------------------------------------------*
FORM INFORMA_DADOS_POR_NOTA  USING P_VT_ALV TYPE ZESDE_CTE_DIST_VT_ALV.
  ...
  READ TABLE IT_05_VT INTO ZESDE_CTE_DIST_VT_ALV WITH KEY TKNUM = P_VT_ALV-TKNUM.

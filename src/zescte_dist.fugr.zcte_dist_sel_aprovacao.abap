...
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(I_CD_CHAVE_CTE) TYPE  ZESDE_CHAVE_DOC_E
*"  EXPORTING
*"     REFERENCE(E_DOMNAME) TYPE  CHAR30
*"     REFERENCE(E_DDTEXT) TYPE  VAL_TEXT
*"  CHANGING
*"     VALUE(E_TP_APROVACAO) TYPE  ZESDE_TIP_APROVACAO
*"  EXCEPTIONS
*"      CTE_NAO_LOC
*"      SEM_AUTORIZACAO
*"      SEM_ESCOLHA
*"----------------------------------------------------------------------
  ...
  "05	Liberar Peso CT-e Ferroviário
  "06	Autorização de Pagamento por Produto
  ...
  "Verifica acesso de Autorização de Pagamento Complemento
...
* Verifica acesso de entrada de peso manula para remessas de formação de lote - ZRFL e ZRDC
* Somente Rodoviário

* Deve ser disponibilizado uma autorização de entrada de peso e data, para modal rodoviário,
* de chegada manual para os fretes de mercadoria de sob ordem de venda de remessa formação
* de lote (ZRFL) ou remessa de DCO (ZRDC), pois as mesmas por default não bloqueadas não podendo
* o usuário entrar de forma manual, porem em alguns casos as cargas não são descarregadas em seu
* local de destino, com isso não terão uma entrada de peso e data de chegada.
  ...
  IF E_TP_APROVACAO IS INITIAL.
    "Verifica acesso de Diferença de Peso CT-e Ferroviário
    AUTHORITY-CHECK OBJECT 'ZACTFTTER' ID 'ZACTFTTER' FIELD '08'.
    IF SY-SUBRC IS INITIAL.
      "08	Liberação de Diferênça de Peso da CT-e
    ...
    IF WA_CTE-CD_MODAL NE '04'.
      "Verifica Autorização de Pagamento - Fardos de Algodão
  ...
  "Verifica acesso de Ignorar a Série na Busca de Nota Fiscal (Modelo 01/04)
  ...
  "07  Liberar Modificação Nota Fiscal (Modelo 01/1A/04)
  IF ( E_TP_APROVACAO IS INITIAL OR E_TP_APROVACAO EQ '07' ) AND ( WA_CTE-CK_FINALIZADO EQ ABAP_FALSE ).
    ...
    IF SY-SUBRC IS INITIAL.
      "07 Liberar Edição/Exclusão/Inserção de Nota Fiscal Modelo 01/1A/04 da CT-e.
      IT_LIBERADO-DOMNAME    = 'ZDM_TIP_APROVACAO'.
  ...
  "08  Liberar Modificação Nota Fiscal (Modelo 55)

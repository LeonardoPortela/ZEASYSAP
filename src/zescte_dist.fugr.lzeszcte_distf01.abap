...
DATA: container       TYPE REF TO cl_gui_custom_container,
      ...
      ck_tp_aprovacao TYPE ZESZDE_TIP_APROVACAO.
...
CONSTANTS: ck_aprovado TYPE ZESZDE_TP_AUTORIZACAO VALUE '01', "Autorizado
           ck_negado   TYPE ZESZDE_TP_AUTORIZACAO VALUE '02'. "Negado
...
*----------------------------------------------------------------------*
FORM autorizacao_cte  USING    p_cd_chave_cte  TYPE ZESZDE_CHAVE_DOC_E
                               p_tp_aprovacao  TYPE ZESZDE_TIP_APROVACAO
                      CHANGING p_cd_aprovacao  TYPE ZESZDE_EST_APROVACAO
                               p_tp_autorizado TYPE ZESZDE_TP_AUTORIZACAO.

  CLEAR: ZESZIB_CTE_DIST_EAP, ck_confirmado, ck_read_only.

  ZESZIB_CTE_DIST_EAP-tp_aprovacao = p_tp_aprovacao.
  ZESZIB_CTE_DIST_EAP-cd_chave_cte = p_cd_chave_cte.

  IF p_tp_autorizado IS NOT INITIAL.
    ck_confirmado = abap_true.
    PERFORM atribuir_usuario_data_hora.
    ZESZIB_CTE_DIST_EAP-tp_autorizado = p_tp_autorizado.
...
FORM atribuir_usuario_data_hora .
  ZESZIB_CTE_DIST_EAP-ds_name_usuario = sy-uname.
  ZESZIB_CTE_DIST_EAP-dt_autorizacao  = sy-datum.
  ZESZIB_CTE_DIST_EAP-hr_autorizacao  = sy-uzeit.
ENDFORM.
...
*----------------------------------------------------------------------*
FORM gravar_autorizacao CHANGING pcd_aprovacao TYPE ZESZDE_EST_APROVACAO ptp_autorizado TYPE ZESZDE_TP_AUTORIZACAO.
  ...
  ZESZIB_CTE_DIST_EAP-cd_aprovacao = pcd_aprovacao.
  ZESZIB_CTE_DIST_EAP-ck_ultimo    = abap_true.
  MODIFY ZESZIB_CTE_DIST_EAP.

  ptp_autorizado = ZESZIB_CTE_DIST_EAP-tp_autorizado.

  "01	Chegada de Documentos
  "02	Complemento
  "03	Travar Pagamento

  UPDATE ZESZIB_CTE_DIST_EAP
     SET ck_ultimo = abap_false
   WHERE tp_aprovacao EQ ZESZIB_CTE_DIST_EAP-tp_aprovacao
     AND cd_chave_cte EQ ZESZIB_CTE_DIST_EAP-cd_chave_cte
     AND ck_ultimo    EQ ZESZIB_CTE_DIST_EAP-ck_ultimo
...
MODULE status_0100 OUTPUT.
  ...
  CALL FUNCTION 'GET_DOMAIN_VALUES'
    EXPORTING
      domname    = 'ZESZDM_TIP_APROVACAO'
    TABLES
      values_tab = it_dd07v.

  READ TABLE it_dd07v WITH KEY domvalue_l = ZESZIB_CTE_DIST_EAP-tp_aprovacao.
...
*    CONCATENATE ZESZIB_CTE_DIST_EAP-CD_CHAVE_CTE ZESZIB_CTE_DIST_EAP-TP_APROVACAO INTO WA_OBJ-OBJKEY.
*
  ...
  CASE ck_bt_tela.
    WHEN icon_allow.
      ...
      IF ck_read_only NE abap_true.
        MESSAGE s154 WITH ZESZIB_CTE_DIST_EAP-cd_chave_cte INTO ck_bt_text.
      ELSE.
        PERFORM texto_tela USING ck_bt_text.
      ENDIF.
      icon_text = 'Aprovar'.
    ...
    WHEN icon_reject.
      ...
      IF ck_read_only NE abap_true.
        MESSAGE s155 WITH ZESZIB_CTE_DIST_EAP-cd_chave_cte INTO ck_bt_text.
      ELSE.
        PERFORM texto_tela USING ck_bt_text.
      ENDIF.
      icon_text = 'Rejeitar'.
...
MODULE user_command_0100 INPUT.
  ...
  CASE ok_code.
    WHEN ok_aprovar.
      ...
      CASE ZESZIB_CTE_DIST_EAP-tp_aprovacao.
    ...
    WHEN ok_confirmar.
      CASE ck_bt_tela.
        WHEN icon_allow.
          ...
          CASE ZESZIB_CTE_DIST_EAP-tp_aprovacao.
            WHEN '03'. "Travar Pagamento
              ZESZIB_CTE_DIST_EAP-tp_autorizado = ck_negado.
            WHEN OTHERS.
              "Chegada de Documentos
              "Autoriza├º├úo de Pagamento Complemento
              "Peso/Data de Chegada Manual
              ZESZIB_CTE_DIST_EAP-tp_autorizado = ck_aprovado.
          ENDCASE.

          CHECK ZESZIB_CTE_DIST_EAP-ck_ultimo EQ abap_true.
          ck_confirmado = abap_true.
          LEAVE TO SCREEN 0.
...
*  ENDIF.
  ...
  CLEAR: ck_read_only, ck_bt_tela, ZESZIB_CTE_DIST_EAP, tl_tlines[].
...
FORM texto_tela  USING p_ck_bt_text.
  ...
  READ TABLE it_dd07v_tp WITH KEY domvalue_l = ZESZIB_CTE_DIST_EAP-tp_aprovacao.

  CALL FUNCTION 'GET_DOMAIN_VALUES'
    EXPORTING
      domname    = 'ZESZDM_TP_AUTORIZACAO'
    TABLES
      values_tab = it_dd07v_tu.

  READ TABLE it_dd07v_tu WITH KEY domvalue_l = ZESZIB_CTE_DIST_EAP-tp_autorizado.

  CASE ZESZIB_CTE_DIST_EAP-tp_aprovacao.
    WHEN '01'. "Chegada de Documentos
      CASE ZESZIB_CTE_DIST_EAP-tp_autorizado.
        WHEN '01'. "Autorizado
          ck_bt_tela = icon_allow.
        WHEN '02'. "Negado
          ck_bt_tela = icon_reject.

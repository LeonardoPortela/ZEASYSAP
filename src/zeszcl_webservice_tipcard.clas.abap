class ZESZCL_WEBSERVICE_TIPCARD definition
  public
  inheriting from ZESZCL_WEBSERVICE
  create public .

*"* public components of class ZESZCL_WEBSERVICE_TIPCARD
...
public section.
  ...
  methods AUTENTICACAO
    importing
      ...
      !I_GRUPO type ZESZDE_DS_GRUPO_EMPRESA
    returning
  ...
  methods XML_AUTENTICA
    importing
      !I_USUARIO type ZESZUSE_WEB
      !I_SENHA type ZESZPASS_WEB
    returning
  ...
  methods CONSULTAR_ARQUIVO_COBRANCA
    importing
      ...
      !I_FATURA type ZESZPFE_NR_LOTE_ADM optional
    exporting
      !E_MSG type CHAR255
  ...
  methods SOLICITA_ROTA
    ...
    exporting
      !E_MSG type CHAR255
      !E_ID_ROTA_ADM type ZESZDE_ID_ROTA_ADM
    exceptions
      ZWEBSERVICE .
...
*"* protected components of class ZESZCL_WEBSERVICE_TIPCARD
*"* do not include other source files here!!!
...
private section.
  ...
  methods LER_XML_STATUS_PARCEIRO
    ...
    returning
      value(E_ZLEST0135) type ZLEST0135 .
*"* private components of class ZESZCL_WEBSERVICE_TIPCARD
*"* do not include other source files here!!!
ENDCLASS.



CLASS ZESZCL_WEBSERVICE_TIPCARD IMPLEMENTATION.
  ...
  METHOD atualizar_rota.
    ...
    DATA: e_chave      TYPE char32,
          ...
          cx_exception TYPE REF TO ZESZCX_WEBSERVICE,
          lc_xml       TYPE string,
          ...
          cd_id_rota   TYPE ZESZDE_ID_ROTA.
    ...
    TRY .
      ...
      CATCH ZESZCX_WEBSERVICE INTO cx_exception .
...
*****************
  ...
  METHOD autenticacao.
    ...
    IF ( sy-subrc EQ 0 ).
      ...
      me->ZESZIF_WEBSERVICE~abrir_conexao( i_http = i_http ).
...
*       ZESZCX_WEBSERVICE=>gera_erro_geral( i_texto = msg_texto ).
*-168836-26.02.2025-JT-fim
      ...
      IF ( sy-subrc EQ 0 ) AND ( NOT xml_node IS INITIAL ).
        ...
        CASE var_valor.
          WHEN: 'ok' OR 'OK'.
            ...
            ELSE.
              RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>erro_no_xml.
            ENDIF.
      ...
      ENDIF.
    ELSE.
      RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING textid = ZESZCX_WEBSERVICE=>autenticacao_nao_encontrado.
    ENDIF.
  ...
  METHOD BUSCAR_ROTAS.
    ...
    DATA: CX_EXCEPTION TYPE REF TO ZESZCX_WEBSERVICE. "Referencia para a Classe de Exception.
    ...
    IF ( SY-SUBRC EQ 0 ).
      ...
      LOOP AT GT_SETLEAF INTO GW_SETLEAF.
        ...
        ELSE.
          ...
          ME->ZESZIF_WEBSERVICE~ABRIR_CONEXAO( VAR_HTTP ).
        ...
        GW_XML_ROTA = ME->ZESZIF_WEBSERVICE~CONSULTAR( I_HTTP = VAR_HTTP
                                                    I_XML  = GW_XML
                                                   ).
...
*****************
    ...
    DATA: GW_ZLEST0086 TYPE ZLEST0086, "Controle da Chave de Autenticação TIPCARD
          ...
          CX_EXCEPTION TYPE REF TO ZESZCX_WEBSERVICE, "Classe da Exception
  ...
  METHOD consultar_arquivo_cobranca.
    ...
    DATA: cx_exception TYPE REF TO ZESZCX_WEBSERVICE.
...
*** Fim - Rubenilson - 17.12.24 - US160867
  ...
  METHOD cons_situacao_transportador.
    ...
    DATA: lc_webservice TYPE REF TO ZESZCL_WEBSERVICE_TIPCARD.
    ...
    IF wa_zlest0137-ck_consulta IS NOT INITIAL OR i_ck_consulta IS NOT INITIAL.
      ...
      LOOP AT it_consultas INTO wa_consultas.
        ...
        lc_webservice->ZESZIF_WEBSERVICE~abrir_conexao( lc_http ).
        ...
        CALL METHOD lc_webservice->ZESZIF_WEBSERVICE~consultar
          EXPORTING
            i_http                     = lc_http
            i_xml                      = lc_xml
          RECEIVING
...
*</retornoConsultaRota>
  ...
  METHOD LER_XML_ROTAS.
    ...
    ELSE.
      "RAISE EXCEPTION TYPE ZESZCX_WEBSERVICE EXPORTING TEXTID = ZESZCX_WEBSERVICE=>SEM_ROTA_UPDATE.
    ENDIF.
  ...
  ENDMETHOD.
   ...
   METHOD processar_arquivo_cobranca.
     ...
     DATA: it_lines_arq       TYPE zde_linha_txt_1000_t,
           ...
           "LC_NR_LOTE_ADM    TYPE ZESZPFE_NR_LOTE_ADM,
           ...
           vg_lote            TYPE  i,
           vg_nm_lote_item    TYPE ZESZPFE_NUMERO_LOTE,
           vg_tipcontabil     TYPE ZESZTIPCONTABIL,
           vg_tipcontabil_itm TYPE ZESZTIPCONTABIL,
           st_lote            TYPE ZESZPFE_NUMERO_LOTE,
           st_lote_aux        TYPE ZESZPFE_NUMERO_LOTE,
...
**          Inclusao de nova linha na tabela e lote itens
  ...
  METHOD SOLICITA_ROTA.
    ...
    "LC_MSG_ADM = ME->ZESZIF_WEBSERVICE~CONSULTAR( I_HTTP = LC_HTTP I_XML = LC_XML ).

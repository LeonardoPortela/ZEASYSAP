class ZESCL_DOC_ELETRONICO definition
  public
  create public .

public section.

  interfaces ZESIF_DOC_ELETRONICO .
...
CLASS ZESCL_DOC_ELETRONICO IMPLEMENTATION.
  ...
  METHOD ZESIF_DOC_ELETRONICO~GET_AGUARDAR.
    ...
    DATA(LC_TEXTO) = |Aguardar Autorização do Documento { ME->ZESIF_DOC_ELETRONICO~AT_DOCUMENTO-DOCNUM } modal { ME->ZESIF_DOC_ELETRONICO~AT_DOCUMENTO-MODEL }|.
    ...
    IF SY-BATCH NE ABAP_TRUE.
      CALL FUNCTION 'SAPGUI_PROGRESS_INDICATOR'
        EXPORTING
          ...
          TEXT       = LC_TEXTO.
    ELSE.
      MESSAGE S047 WITH ME->ZESIF_DOC_ELETRONICO~AT_DOCUMENTO-DOCNUM ME->ZESIF_DOC_ELETRONICO~AT_DOCUMENTO-MODEL.
    ENDIF.
...
*
    ...
    WHILE LC_CICLOS LE ME->ZESIF_DOC_ELETRONICO~AT_QTD_CICLOS AND ME->ZESIF_DOC_ELETRONICO~AT_INFO_DOC_ELETRONICO-ACTION_REQU EQ ABAP_FALSE.

      PERCENTAGE = ( LC_CICLOS / ME->ZESIF_DOC_ELETRONICO~AT_QTD_CICLOS ) * 100.
      ...
      WAIT UP TO ME->ZESIF_DOC_ELETRONICO~AT_QTD_SEGUNDOS SECONDS.
...
*    ENDCASE.
  ...
  method ZESIF_DOC_ELETRONICO~GET_XML_EVENTO_GRC.
...
*          OTHERS          = 2.
  ...
  method ZESIF_DOC_ELETRONICO~IMPRIMIR_DOCUMENTO_AUX.

    DATA: LVA_LINK_PDF  TYPE STRING,
          LVA_NODE_DATA TYPE BXMNODES-URL.

    DATA: OB_WEB_SERVICE TYPE REF TO ZESCL_WEBSERVICE.
    ...
    CASE I_CHAVE+20(2).
      WHEN '55'.
        ...
        ELSEIF WL_NFE_DIST_TER-ID_SIMETRYA IS NOT INITIAL.
          ...
          CATCH ZESCX_WEBSERVICE INTO DATA(LC_EXCEPTION).
            RAISE EXCEPTION TYPE ZESCX_NFE_INBOUND_EXCEPTION
              EXPORTING
                TEXTID = VALUE #(  MSGNO = ZESCX_NFE_INBOUND_EXCEPTION=>ZCX_SEM_WEBSERVICE_DANFE-MSGNO
                                   MSGID = ZESCX_NFE_INBOUND_EXCEPTION=>ZCX_SEM_WEBSERVICE_DANFE-MSGID )
                MSGTY  = 'E'
                MSGNO  = ZESCX_NFE_INBOUND_EXCEPTION=>ZCX_SEM_WEBSERVICE_DANFE-MSGNO
                MSGID  = ZESCX_NFE_INBOUND_EXCEPTION=>ZCX_SEM_WEBSERVICE_DANFE-MSGID.
          ENDTRY.
      ...
      WHEN '57'.
        ...
        IF WL_CTE_DIST_TER-ID_SIMETRYA IS NOT INITIAL.
          ...
          CATCH ZESCX_WEBSERVICE INTO LC_EXCEPTION.
             RAISE EXCEPTION TYPE ZESCX_CTE_INBOUND
              EXPORTING
                TEXTID = VALUE #(  MSGNO = ZESCX_CTE_INBOUND=>ZCX_SEM_WEBSERVICE_DACTE-MSGNO
                                   MSGID = ZESCX_CTE_INBOUND=>ZCX_SEM_WEBSERVICE_DACTE-MSGID )
                MSGTY  = 'E'
                MSGNO  = ZESCX_CTE_INBOUND=>ZCX_SEM_WEBSERVICE_DACTE-MSGNO
                MSGID  = ZESCX_CTE_INBOUND=>ZCX_SEM_WEBSERVICE_DACTE-MSGID.
          ENDTRY.
